...
const SkeletonTableRow = ({ delay }: { delay: number }) => (
  ...
  >
    ...
    <div className="flex-[1.2] flex items-center gap-2">
      <Skeleton className="h-8 w-full rounded-lg bg-foreground/10" />
      <Skeleton className="h-8 w-16 rounded-lg bg-foreground/10" />
    </div>
    <div className="flex-1 space-y-1.5">
      <Skeleton className="h-5 w-20 rounded-full bg-foreground/10" />
      <Skeleton className="h-3 w-28 rounded bg-foreground/8" />
    </div>
  </div>
);
