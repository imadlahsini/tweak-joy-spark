...
const yearPlan = plans.find((p) => p.slug === "1-year-special")!;
const MONTHLY_PRICE = 14.99;
const FULL_YEAR_PRICE = MONTHLY_PRICE * 12; // $179.88
...
const YearSpecialCheckout = () => {
  ...
  const onSubmit = async (formData: FormData) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("create-checkout-session", {
        body: {
          priceId: yearPlan.priceId,
          customerEmail: formData.email,
          customerName: formData.name,
          planSlug: yearPlan.slug,
        },
      });
  ...
  const savings = FULL_YEAR_PRICE - yearPlan.monthlyPrice;
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 md:gap-8">
                ...
                >
                  <div className="inline-flex flex-col items-center md:items-end gap-1 bg-background/10 backdrop-blur-sm rounded-2xl p-5 sm:p-6">
                    ...
                    >
                      ${yearPlan.monthlyPrice.toFixed(2)}
                    </motion.span>
            ...
            <div className="rounded-2xl border border-border/60 bg-card/80 backdrop-blur-sm p-5 sm:p-6 shadow-soft sticky top-8">
              ...
              <div className="space-y-3 mb-5">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-muted-foreground">1 Year SEO — Special Rate</span>
                  <div className="flex items-center gap-2">
                    <span className="text-xs line-through text-muted-foreground/50">${FULL_YEAR_PRICE.toFixed(2)}</span>
                    <span className="font-medium text-foreground">${yearPlan.monthlyPrice.toFixed(2)}</span>
                  </div>
                </div>
              </div>
...
export default YearSpecialCheckout;
